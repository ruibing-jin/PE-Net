...
from dataset import data_loader as data_loader
from networks.cnn_a import CNN_A
from networks.cnn_b import CNN_B
from networks.cnn_c import CNN_C
from networks.cnn_d import CNN_D
from networks.cnn_c_pe import CNN_PE
...
if __name__ == "__main__":
	...
	data_iter = data_loader.CMPDataIter(config.data.root, config.data.set, config.data.max_rul, config.data.seq_len, config.net.name)
	...
	if config.net.name.lower() == 'cnn_a':
		sym_net = CNN_A(num_hidden = config.net.num_hidden, input_dim = config.net.input_dim, aux_dim = config.net.aux_dim)
	elif config.net.name.lower() == 'cnn_b':
		sym_net = CNN_B(num_hidden = config.net.num_hidden, input_dim = config.net.input_dim, aux_dim = config.net.aux_dim)
	elif config.net.name.lower() == 'cnn_c':
		sym_net = CNN_C(num_hidden = config.net.num_hidden, input_dim = config.net.input_dim, aux_dim = config.net.aux_dim)
	elif config.net.name.lower() == 'cnn_d':
		sym_net = CNN_D(num_hidden = config.net.num_hidden, input_dim = config.net.input_dim, aux_dim = config.net.aux_dim)
	elif config.net.name.lower() == 'cnn_pe':
		sym_net = CNN_PE(num_hidden = config.net.num_hidden, input_dim = config.net.input_dim, aux_dim = config.net.aux_dim)
	else:
		raise NotImplementedError(config.net.name.lower())
