...
class Callback(object):
    ...
    def header(self, epoch=None, batch=None, epoch_num=None, prefix=None):
        str_out = ""
        if self.with_header:
            if epoch is not None:
                str_out += "{:s}-Epoch[{:s}/{:s}]".format(("%s"%prefix), ("%d"%epoch), ("%d"%epoch_num))
...
class CallbackList(Callback):
    ...
    def __call__(self, epoch=None, batch=None, silent=False, epoch_num=None, prefix=None, **kwargs):
        str_out = self.header(epoch, batch, epoch_num, prefix)
...
class SpeedMonitor(Callback):
    ...
    def __init__(self, logger=None, with_header=False):
    ...
    def __call__(self, sample_elapse, update_elapse=None, epoch=None, batch=None, silent=False, **kwargs): 
        ...
        if sample_elapse is not None:
            ...
            if update_elapse is not None:
                ...
                str_out += "Speed {:.1f} (+{:.1f}) sample/sec ".format(sample_freq, update_freq-sample_freq)
            else:
